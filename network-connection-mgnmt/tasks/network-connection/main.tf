...
resource "ipm_network_connection" "network_connection" {
  ...
  config = {
    ...
    service_mode = each.value.service_mode != null ? each.value.service_mode: var.nc_profiles[each.value.nc_profile].service_mode 
    mc = each.value.mc != null ? each.value.mc: var.nc_profiles[each.value.nc_profile].mc 
    outer_vid = each.value.outer_vid != null ? each.value.outer_vid: var.nc_profiles[each.value.nc_profile].outer_vid 
    implicit_transport_capacity = each.value.implicit_transport_capacity != null ? each.value.implicit_transport_capacity: var.nc_profiles[each.value.nc_profile].implicit_transport_capacity 
    labels = each.value.labels != null ? each.value.labels: var.nc_profiles[each.value.nc_profile].labels 
  }
  endpoints = [ for endpoint in each.value.endpoints : 
              {
                config = {
                  ...
                  capacity = endpoint.capacity != null ? endpoint.capacity : var.nc_profiles[each.value.nc_profile].endpoint_capacity
              }
  }]
}
