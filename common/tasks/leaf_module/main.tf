...
resource "ipm_leaf_module" "leaf_modules" {
  ...
  config = {
    selector = each.value.id == null ? (each.value.name == null ? (each.value.mac_address == null ? (each.value.serrial_number == null ? (each.value.host_name == null ? (each.value.host_port_id == null ? (each.value.host_sys_name == null ? (each.value.host_port_source_mac == null ? null : { host_port_selector_by_port_source_mac = { port_source_mac = each.value.port_source_mac } }) : { host_port_selector_by_sys_name = { sys_name = each.value.host_sys_name, port_id = each.value.host_port_id, port_id_subtype = each.value.host_port_id_subtype } }) : { host_port_selector_by_port_id = { chassis_id = each.value.host_chassis_id, chassis_id_subtype = each.value.host_chassis_id_subtype, port_id = each.value.host_port_id, port_id_subtype = each.value.host_port_id_subtype } }) : { host_port_selector_by_name = { host_name = each.value.host_name, host_port_name = each.value.host_port_name } }) : { module_selector_by_module_serial_number = { module_serial_number = each.value.serial_number } }) : { module_selector_by_module_mac = { module_mac = each.value.mac_address } }) : { module_selector_by_module_name = { module_name = each.value.name } }) : { module_selector_by_module_id = { module_id = each.value.id }
    }
    module = {
