...
resource "ipm_transport-capacity" "transport-capacities" {
  for_each = { for tc in var.transport-capacities : tc.name => tc }
  config = {
    name = each.value.name
    capacity_mode = each.value.capacity_mode != null ? each.value.capacity_mode: var.tc_profiles[each.value.tc_profile].capacity_mode 
    labels = each.value.labels != null ? each.value.labels: var.tc_profiles[each.value.tc_profile].labels 
  }
  endpoints = [ for endpoint in each.value.endpoints : 
              {
                config = {
                  ...
                  capacity = endpoint.capacity != null ? endpoint.capacity : var.tc_profiles[each.value.tc_profile].endpoint_capacity
              }
  }]
}
